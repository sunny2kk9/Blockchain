...
contract SimpleLending is Ownable {
    ...
    function repay(uint256 amount) external {
        require(amount > 0 && debt[msg.sender] > 0);
        uint256 pay = amount > debt[msg.sender] ? debt[msg.sender] : amount;
        token.safeTransferFrom(msg.sender, address(this), pay);
        debt[msg.sender] -= pay;
    }

    function withdrawCollateral(uint256 amount) external {
        require(amount > 0 && collateralETH[msg.sender] >= amount);
        require(debt[msg.sender] == 0);
    ...
    function availableToBorrow(address user) external view returns (uint256) {
        ...
        if (maxBorrow <= debt[user]) return 0;
